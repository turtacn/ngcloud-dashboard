export default {
  help: {
    cloudregionZone: `
      ...
      如果您的应用需要较高的容灾能力，建议您将虚拟机 ECS 实例部署在同一地域的不同可用区内。
    ...
    cloudaccountAutoCreateProject: `
      <div>若线上云账号上面已设置项目信息，我们会自动创建与云上项目名称相同的项目（若与本地项目重名，系统默认追加“-1”）</div>
    `,
    useCasProject: `
    ...
    shutdownStopCharging: `关机后，请注意以下信息：<br />
                          * 关机后，将释放CPU、内存，并停止对CPU、内存的计量，所关联硬盘、镜像、公网带宽(如需计量)依然计量<br />
    ...
    billResourceSharedTip: '启用后，将包年包月的资源费用账单分摊到具体使用日期，分摊后总费用 = 按量计算费用 + 包年包月分摊后费用',
