<template>
  <div>
      ...
      <a-col :span="9" class="d-flex">
        ...
        <a-form-item class="mr-1" :style="{ width: `calc(100% - ${i === 0 ? 0 : 70}px)` }">
          <base-select
            ...
            :select-props="{ placeholder: $t('logging.text_109'), allowClear: true, loading }" />
        </a-form-item>
      </a-col>
      <a-col :span="3">
      ...
      <a-col :span="12" class="d-flex">
        <a-form-item :style="{ width: `calc(100% - ${i === 0 ? 0 : 20}px)` }">
          <base-select
            ...
            filterable
            :disabled="disabled"
            :select-props="{ placeholder: $t('logging.text_110'), allowClear: true, loading }" />
        </a-form-item>
    ...
    <div class="d-flex align-items-center" v-if="!disabled">
      ...
      <a-button type="link" @click="add">{{ $t('logging.logging_add_filters') }}</a-button>
    </div>
  </div>
</template>
...
import { filterKeyMap } from '@Logging/constants'
    ...
    tagKeyOpts () {
      if (R.is(Array, this.metricInfo.tag_key)) {
        return this.metricInfo.tag_key.map(v => {
          ...
          const label = zh ? `${zh} (${v})` : (v.startsWith('user:') ? v.replace('user:', this.$t('logging.tag_key')) : v)
          return {
            key: v,
            label,
          }
