...
export default {
  ...
  methods: {
    ...
    handleDelete () {
      this.createDialog('DeleteLoggingDashboardChart', {
    ...
    handleClone () {
      this.createDialog('CloneLoggingDashboardChart', {
    ...
    async fetchData (metric_query) {
      try {
        ...
        const { data: { series = [] } } = await new this.$Manager('unifiedloggings', 'v1').performAction({ id: 'query', action: '', data, params: { $t: getRequestT() } })
        return series
      } catch (error) {
        throw error
      }
