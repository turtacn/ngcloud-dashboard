    ...
    <a-breadcrumb style="padding: 5px;">
      ...
      <a-breadcrumb-item v-for="(item, index) in navs" :key="item.id">
        <template v-if="index === lastIndex">
          <span class="logging-overview-breadcrumb-span">{{ locationTitle(item.location) }}</span>
        </template>
        <template v-else>
          <a class="logging-overview-breadcrumb-link" @click="changeNav(index)">{{ item.location }}</a>
        </template>
      </a-breadcrumb-item>
    </a-breadcrumb>
    <div
      ...
      <a-icon type="arrow-left" class="anticon anticon-arrow-left logging-overview-breadcrumb-link" :style="{ cursor: 'pointer', }" @click="changeNav(navs.length-2)" />
...
export default {
  ...
  methods: {
    locationTitle: function (base) {
      return this.$t('logging.overview.location', [base])
    },
    changeNav (i) {
...
.logging-overview-breadcrumb-span {
  color: @text-color-secondary !important;
}

.logging-overview-breadcrumb-link {
