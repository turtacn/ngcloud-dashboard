<template>
  <a-form :form="form" layout="inline">
      ...
      <a-form-item style="margin-right: 8px;" class="ml-2" :label="$t('logging.overview.aggregate')">
  ...
  computed: {
    ...
    dimentions () {
      ...
      scopeLevel > 2 && ret.push({
        ...
        id: 'system',
        name: 'system',
        label: this.$t('logging.view_system'),
      })
  ...
  methods: {
    ...
    async fetchChartData (field, formValues) {
      ...
      try {
        ...
        const { data: { series = [] } } = await new this.$Manager('unifiedloggings', 'v1').performAction({ id: 'query', action: '', data })
