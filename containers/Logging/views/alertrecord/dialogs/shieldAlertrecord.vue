<template>
  <base-dialog @cancel="cancelDialog">
    <div slot="header">{{ $t('logging.alerts.shield.shield') }}</div>
    <div slot="body" class="alertrecord-detail">
      <dialog-selected-tips :name="$t('logging.text_1')" :count="params.data.length" :action="$t('logging.alerts.shield.shield')" />
      <dialog-table :data="params.data" :columns="params.columns.slice(0, 4)" />
      <a-form :form="form" hideRequiredMark>
        <a-form-item :label="$t('logging.alerts.shield.time')" v-bind="formItemLayout">
        ...
        <a-form-item :label="$t('logging.alerts.shield.reason')" v-bind="formItemLayout">
          <a-input v-decorator="decorators.description" :placeholder="$t('common.tips.optional_input', [$t('logging.alerts.shield.reason')])" />
        </a-form-item>
      </a-form>
    </div>
    <div slot="footer">
    ...
    async handleConfirm () {
      ...
      try {
        ...
        const params = {
          ...
          res_id: this.params.data[0].logging_resource_id,
