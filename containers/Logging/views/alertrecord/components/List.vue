...
import { levelMaps } from '@Logging/constants'
...
import { strategyColumn, levelColumn } from '@Logging/views/commonalert/utils'
...
export default {
  ...
  data () {
    return {
      ...
      unrecoverList: this.$list.createList(this, this.listOptions('loggingresourcealerts')),
      exportDataOptions: {
        items: [
          { key: 'alert_name', label: this.$t('logging.text_99') },
          { key: 'created_at', label: this.$t('logging.text_14') },
          { key: 'state', label: this.$t('common.status') },
          { key: 'res_type', label: this.$t('logging.text_97') },
          { key: 'alert_rule', label: this.$t('logging.strategy_detail') },
          { key: 'level', label: this.$t('logging.level') },
  ...
  computed: {
    ...
    singleActions () {
      ...
      if (this.alertType === 'un-recovered') {
        actions.push({
          label: this.$t('logging.alerts.shield.shield'),
          ...
          meta: (obj) => {
            ...
            //   return {
            //     validate: false,
            //     tooltip: this.$t('logging.alerts.shield.tips'),
            //   }
            // }
            if (obj.is_set_shield === true) {
              return {
                validate: false,
                tooltip: this.$t('logging.alerts.shield.tips2'),
              }
            }
            return ret
          },
  ...
  methods: {
    ...
    filters () {
      const options = {
        name: getNameFilter({ field: 'name', label: this.$t('logging.text_99') }),
        description: getDescriptionFilter(),
        level: {
          label: this.$t('logging.level'),
          dropdown: true,
        ...
        created_at: getTimeRangeFilter({ label: this.$t('logging.text_14'), field: 'trigger_time' }),
      }
    ...
    listOptions (resource) {
      return {
        id: this.listId,
        idKey: resource === 'loggingresourcealerts' ? 'row_id' : 'id',
        resource: resource,
        apiVersion: 'v1',
            ...
            slots: {
              ...
              content: ({ row }) => {
                const columns = [
                  ...
                  {
                    field: 'value_str',
                    title: row.state === 'ok' ? this.$t('logging.text_106') : this.$t('logging.text_105'),
                    align: 'right',
    ...
    listColumns (alertType) {
      return [
        ...
        getTimeTableColumn({ field: 'trigger_time', title: this.$t('logging.text_14') }),
