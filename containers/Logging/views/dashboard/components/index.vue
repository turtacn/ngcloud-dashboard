    ...
    <div v-if="isEmpty">
      <data-empty style="padding-top: 200px;">
        <a-button type="primary" @click="handleCreateDashboard">
          {{ $t('logging.dashboard.dialog.create') }}
        </a-button>
      </data-empty>
    </div>
    <div v-else>
      <a-row :gutter="8">
        <a-col :span="10">
          <a-row type="flex" :gutter="8" justify="start">
            <a-col style="padding-top: 6px;padding-bottom: 6px;">{{ $t('logging.dashboard.label') + ':' }}</a-col>
            <a-col flex="auto">
              <base-select
                  filterable
...
import DashboardCards from '@Logging/components/LoggingCard/DashboardCards'
...
export default {
  ...
  methods: {
    handleCreateDashboard () {
      this.createDialog('CreateLoggingDashboard', {
    ...
    handleClone () {
      this.createDialog('CloneLoggingDashboard', {
    ...
    handleDelete () {
      this.createDialog('DeleteLoggingDashboard', {
        data: this.dashboards.filter((item) => { return item.id === this.dashboardId }),
        refresh: this.switchDashboard,
        columns: [getNameDescriptionTableColumn(), getProjectDomainTableColumn(), getProjectTableColumn()],
      })
    ...
    editChart ({ id }) {
      if (!id) return
      this.$router.push({
        path: `/logging-dashboard/${id}/update`,
        query: {
          dashboard: this.dashboardId,
          ...this.extraParams,
        },
