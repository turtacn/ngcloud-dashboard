<template>
  <div>
    <page-header :title="this.panelId ? $t('logging.dashboard.dialog.project.update') : $t('logging.dashboard.dialog.project.create')" />
      ...
      <a-row>
        ...
        <a-col class="line mb-5"
          ...
          <a-card v-if="!seriesList.length && loadingList[0]"
                  class="explorer-logging-line d-flex align-items-center justify-content-center">
            <loader :loading="true" />
          </a-card>
        </a-col>
      </a-row>
...
export default {
  ...
  methods: {
    ...
    goback () {
      this.$router.push({ path: '/logging-dashboard', query: { dashboard_id: this.$route.query.dashboard } })
    },
    async handleConfirm () {
      this.loading = true
