<template>
  <a-row>
    <a-col :md="{ span: 24 }" :lg="{ span: 22 }" :xl="{ span: 16 }"  :xxl="{ span: 11 }" class="mb-5">
      <logging-forms @refresh="refresh" @remove="remove" @resetChart="resetChart" :timeRangeParams="timeRangeParams" @mertricItemChange="mertricItemChange" />
    </a-col>
    <a-col class="line mb-5" :md="{ span: 24 }" :lg="{ span: 22 }" :xl="{ span: 16 }" :xxl="{ span: 12, offset: 1 }">
      ...
      <div v-for="(item, i) in seriesList" :key="i">
        <logging-line :loading="loadingList[i]" :description="seriesDescription[i]" :metricInfo="metricList[i][0]" class="mb-3" @chartInstance="setChartInstance" :series="item" :timeFormatStr="timeFormatStr" :pager="seriesListPager[i]" @pageChange="pageChange">
          <template #extra>
            <a-button class="mr-3" type="link" @click="handleSave(metricList[i], seriesDescription[i])">{{ $t('common.save') }}</a-button>
          </template>
        </logging-line>
      </div>
      <a-card v-if="!seriesList.length && loadingList[0]" class="explorer-logging-line d-flex align-items-center justify-content-center">
        <loader :loading="true" />
      </a-card>
    </a-col>
  </a-row>
...
import LoggingForms from '@Logging/sections/ExplorerForm'
  ...
  methods: {
    ...
    async fetchData (metric_query, limit, offset) {
      try {
        ...
        const { data: resdata } = await new this.$Manager('unifiedloggings', 'v1').performAction({ id: 'query', action: '', data, params: { $t: getRequestT() } })
        return resdata
      } catch (error) {
        throw error
      }
    ...
    handleSave (mq, desc) {
      this.createDialog('CreateLoggingDashboardChart', {
        name: desc.title,
        metric_query: mq,
