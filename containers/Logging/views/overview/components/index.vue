<template>
  <div class="overview-index">
    ...
    <a-row>
      <div class="logging-overview-chart mb-2">
        <div class="title-wrapper">
          <div class="title">
            {{ $t('logging.dashboard.overview.title') }}
            <help-tooltip name="loggingDashboardOverviewTips" />
          </div>
        </div>
    ...
    <a-row>
      <a-col :span="8">
        <div class="logging-overview-chart mb-2">
          <div class="title-wrapper">
            <div class="title">{{ $t('logging.overview_alert_sum') }}</div>
          </div>
          <overview-ring
            yAxisFormat="0"
            chartHeigth="299px"
            ...
            :title="$t('logging.overview_alert_sum_pie')"
            :subtitle="ringChart.subtitle"
            :loading="ringChart.loading"
            :exportExcelColumns="exportExcelColumns"
            :exportName="$t('logging.overview_alert_sum')" />
        </div>
      </a-col>
      <a-col :span="16">
        <div class="logging-overview-chart mb-2">
          <div class="title-wrapper">
            <div class="title">{{  $t('logging.overview_alert_trend') }}</div>
          </div>
          <overview-line
            yAxisFormat="0"
            chartHeigth="299px"
...
<script>
import OverviewRing from '@Logging/components/LoggingCard/sections/chart/ring'
import OverviewLine from '@Logging/components/LoggingCard/sections/chart/line'
import OverviewCard from '@Logging/components/LoggingCard/OverviewCard'
import OverviewNav from '@Logging/components/LoggingCard/sections/nav'
  ...
  methods: {
    ...
    tabChartData (rawDatas) {
      ...
      if (rawDatas && rawDatas.length > 0) {
        ...
        rawDatas.map((item) => {
          ...
          const columnName = item.raw_name ? this.$t(`dictionary.${item.raw_name}`) : this.$t('logging.overview_alert.undefined')
    ...
    async fetchTabChartData (measurement, field) {
      try {
        ...
        const { data: { series = [] } } = await new this.$Manager('unifiedloggings', 'v1').performAction({ id: 'query', action: '', data, params: { $t: new Date().getSeconds() } })
