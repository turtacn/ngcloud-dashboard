        ...
        slots: {
          ...
          content: ({ row }) => {
            const columns = [
              ...
              {
                field: 'value_str',
                title: this.$t('logging.text_105'),
                align: 'right',
                formatter: ({ row }) => {
                  const val = _.get(row, 'data.value_str') || '-'
                  if (val === 'nodata') return this.$t('logging.text_116')
                  return val
                },
              },
              {
