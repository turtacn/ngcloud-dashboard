...
import { metric_zh, alertStrategyMaps, preiodMaps, levelMaps } from '@Logging/constants'
...
export const strategyColumn = (field = 'common_alert_metric_details', title = i18n.t('logging.strategy_detail')) => ({
  field,
  title,
  minWidth: 120,
  slots: {
    default: ({ row }, h) => {
      ...
      if (filters.length > 0) {
        filterNode = (
          <a-tag class="w-100">
            <div>{ i18n.t('logging.text_101') }: </div>
...
export const projectTableColumn = {
  field: 'project',
  title: i18n.t('logging.text00015'),
  slots: {
    default: ({ row }, h) => {
      if (!row.tenant && !row.project_domain) {
        return i18n.t('logging.text00024')
      }
      const domain = row.project_domain || row.domain
      if (!row.tenant && domain) {
        return `${domain}${i18n.t('logging.text00025')}`
      }
      if (row.tenant && domain) {
        return [
...
export function getMetircAlertUtil (row, field) {
  ...
  if (row[field] && ((R.type(row[field]) === 'Array') || R.type(row[field]) === 'Object') && !R.isEmpty(row[field])) {
    ...
    const alert_duration = row.alert_duration ? i18n.t('logging.list.duration.label', [row.alert_duration]) : row[field].alert_duration ? i18n.t('logging.list.duration.label', [row[field].alert_duration]) : ''
    ...
    strategy = i18n.t('logging.text_6', [measurement, metric, reduce, alert_duration, comparator, txt])
    if (detail.condition_type === 'nodata_query') { // 系统上报数据为空
      strategy = i18n.t('logging.text_108', [alert_duration])
    }
    if (preiod) {
      preiod = preiod.replace(i18n.t('logging.text_103'), '')
      strategy += `${i18n.t('logging.text_102', [preiod])}`
    }
    ...
    if (silent_period) {
      ...
      if (p.endsWith('m')) {
        ...
        if (pi && pi >= 60 && pi % 60 === 0) {
          p = i18n.t('logging.duration.silent.hour', [pi / 60])
        } else {
          p = i18n.t('logging.duration.silent.minute', [p.replace('m', '')])
        }
      } else if (p.endsWith('h')) {
        p = i18n.t('logging.duration.silent.hour', [p.replace('h', '')])
      }
      strategy += `${i18n.t('logging.commonalerts.list.silent', [p])}`
    }
