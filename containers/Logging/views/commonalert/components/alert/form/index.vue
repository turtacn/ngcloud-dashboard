<template>
  <a-form v-bind="formItemLayout" :form="form.fc">
    ...
    <a-form-item :label="$t('logging.condition')" class="mb-0">
    ...
    <a-form-item :label="$t('logging.commonalerts.form.column.silent')" class="mb-0">
      <a-form-item class="mr-1">
        ...
        <template slot="title">
          <span>{{ $t('logging.commonalerts.form.column.silent.tips') }}</span>
        </template>
        </a-tooltip>
      </a-form-item>
    </a-form-item>
    <a-form-item :label="$t('logging.level')">
    ...
    <a-form-item :label="$t('logging.notification_type')">
      <a-checkbox-group v-decorator="decorators.notify_type">
        <a-checkbox value="recipient">{{ $t('logging.recipient') }}</a-checkbox>
        <a-checkbox v-if="currentScope!=='project'" value="role">{{ $t('logging.role') }}</a-checkbox>
        <a-checkbox value="robot">{{ $t('logging.text_11') }}</a-checkbox>
      </a-checkbox-group>
    </a-form-item>
    <a-form-item v-if="notifyTypes.includes('recipient')" :label="$t('logging.recipient')" style="white-space: nowrap;">
      <template #extra style="width: 1200px;" v-if="scope !== 'project'">
        <i18n tag="div" path="logging_text00001">
          <help-link slot="new" href="/contact">{{$t('logging.text_15')}}</help-link>
        </i18n>
      </template>
      <base-select
    ...
    <a-form-item v-if="notifyTypes.includes('recipient')" :label="$t('logging.channel')">
      <a-checkbox-group
        ...
        <a-checkbox
          ...
          <template v-if="v.disabled">
            <a-tooltip placement="top">
              <template slot="title">
               {{$t('logging.commonalerts.channel.disable.tips', [v.label])}}
              </template>
              {{ v.label }}
            </a-tooltip>
          </template>
    ...
    <a-form-item v-if="notifyTypes.includes('role') && currentScope!=='project'" :label="$t('logging.role')">
      <base-select
    ...
    <notify-types
      ...
      :label="$t('logging.text_11')"
      :placeholder="$t('common.tips.select', [$t('logging.text_11')])"
...
import { levelMaps, metric_zh } from '@Logging/constants'
...
export default {
  ...
  data () {
    ...
    return {
      ...
      silentOpts: [
        { key: '5m', label: this.$t('logging.duration.silent.minute', [5]) },
        { key: '10m', label: this.$t('logging.duration.silent.minute', [10]) },
        { key: '15m', label: this.$t('logging.duration.silent.minute', [15]) },
        { key: '30m', label: this.$t('logging.duration.silent.minute', [30]) },
        { key: '60m', label: this.$t('logging.duration.silent.hour', [1]) },
        { key: '120m', label: this.$t('logging.duration.silent.hour', [2]) },
        { key: '180m', label: this.$t('logging.duration.silent.hour', [3]) },
        { key: '360m', label: this.$t('logging.duration.silent.hour', [6]) },
        { key: '720m', label: this.$t('logging.duration.silent.hour', [12]) },
        { key: '1440m', label: this.$t('logging.duration.silent.hour', [24]) },
      ],
      projectItem: {},
      metricLoading: false,
      ...
      label: this.$t('logging.text00015'),
    }
  },
  computed: {
  ...
  methods: {
    ...
    async getMeasurement (params = {}) {
      try {
        ...
        this.formScopeParams = params
        const { data: { res_type_measurements, res_types } } = await new this.$Manager('unifiedloggings', 'v1').get({ id: 'measurements', params: { ...params, ...this.timeRangeParams } })
    ...
    async validate () {
      const loggingParams = this.toParams(false)
